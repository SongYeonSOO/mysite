...
<script type="text/javascript">
	...
	var fetchList = function() {
		$.ajax({
			...
			url : "${pageContext.request.contextPath}/guestbook?a=ajax-list&p="
					+ page,
			type : "get",
	...
	$(function() {
		...
		$("#form-insert").submit(
				function(event) {
					...
					var pass = $("#pass").val(); // value가 아니라 val이라는 ftn
					var message = $("#message").val(); // value가 아니라 val이라는 ftn

					//form reset
					/* 	console.log("wow : "+$("#form-insert")[0]);
						console.log("wow2 : "+$("#form-insert"));
					...
					$.ajax({
						...
						type : "post",
						dataType : "json", //이거 안쓰면 parsing error난다! listaction이 실행되기 때문
						data : "a=ajax-insert&name=" + name + "&pass=" + pass
								+ "&message=" + message,
						success : function(response) {
			...
			buttons : {
				"삭제" : function() {
					...
					$.ajax({
						...
						type : "post",
						dataType : "json", //이거 안쓰면 parsing error난다! listaction이 실행되기 때문
						data : "a=ajax-delete&no=" + no + "&pass=" + passwd,
						success : function(response) {
		...
		<div title="메세지 비밀번호 입력">
			...
			<form id="dialog-form">
				<label for="password">Password</label> <input type="hidden"
					id="del-no" value=""><input type="password" name="password"
					id="del-password" value="xxxxxxx"
